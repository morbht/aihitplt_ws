...
class YoloDetect:
    ...
    def detect(self, frame):
        try:
            ...
            # 获取原始图像尺寸
            original_height, original_width = frame_processed.shape[:2]
            
            # 设置目标显示尺寸（根据你的屏幕分辨率调整）
            # 假设你的屏幕是1920x1080，显示窗口设为屏幕的80%
            max_display_width = 768  # 1920 * 0.8
            max_display_height = 432  # 1080 * 0.8
            ...
            if scale_ratio < 0.95:  # 如果图像比显示区域大很多
                new_width = int(original_width * scale_ratio)
                new_height = int(original_height * scale_ratio)
                frame_display = cv.resize(frame_processed, (new_width, new_height), interpolation=cv.INTER_AREA)
            else:
                frame_display = frame_processed
            # ====================================
            
            # 显示图像（使用缩放后的图像）
            cv.imshow('YOLO Detection', frame_display)
            cv.waitKey(1)
