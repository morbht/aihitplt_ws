...
class AGVControlGUI:
    ...
    def create_shangzhuang_content(self):
        ...
        self.delivery_test = self.create_button(
            "送物模块硬件系统测试",
            "delivery_test",
            content_frame,
            command = self.toggle_delivery_test           
    ...
    def toggle_delivery_test(self):
        """test_test_test"""
        feature = "delivery_test"
        button_info = self.button_states[feature]
        
        if button_info['active']:
            # 关闭键盘控制
            success = self.stop_ros_launch('aihitplt_delivery_gui.launch')
            if success:
                button_info['button'].config(text="送物模块硬件测试系统", bg='#e0e0e0', fg='black')
                button_info['active'] = False
                print("关闭送物模块硬件测试系统")
        
        else:
            # 启动键盘控制
            success = self.start_ros_launch('aihitplt_delivery_gui.launch', '送物模块硬件测试系统')
            if success:
                button_info['button'].config(text="关闭送物模块硬件测试系统", bg='#4CAF50', fg='white')
                button_info['active'] = True
                print("送物模块硬件测试系统")            
