...
{
  ros::init(argc, argv, "aihitplt_robot"); //ROS initializes and sets the node name //ROS初始化 并设置节点名称 
...
//设置灯带颜色服务
bool turn_on_robot::Set_LightRgb_Callback(aihitplt_bringup::SetRgb::Request &req,aihitplt_bringup::SetRgb::Response &res)
{
...
***************************************/
void turn_on_robot::Recharge_Flag_Callback(const std_msgs::Int8 &Recharge_Flag)
{
  AutoRecharge=Recharge_Flag.data;
}

//底盘安全开启或关闭的标志位
void turn_on_robot::Security_Callback(const std_msgs::Int8 &Security_Flag)
{
...
void turn_on_robot::Publish_Voltage()
{
    ...
    static float Count_Voltage_Pub=0, time_period;
    static ros::Time Last_Time = ros::Time::now();

    time_period=(ros::Time::now() - Last_Time).toSec();
...
void turn_on_robot::Publish_distance()
{
  ...
  if(distance.C>=0||distance.C<=6) distance_msg.distanceC = distance.C;
  if(distance.D>=0||distance.D<=6) distance_msg.distanceD = distance.D;
  if(distance.E>=0||distance.E<=6) distance_msg.distanceE = distance.E;
  if(distance.F>=0||distance.F<=6) distance_msg.distanceF = distance.F;
  //if(distance.G>=0||distance.G<=6) distance_msg.distanceG = distance.G;
  //if(distance.H>=0||distance.H<=6) distance_msg.distanceH = distance.H;
...
{ 
  ...
  uint8_t i=0,check=0,check2=0,check3=0, error=1,error2=1,error3=1,Receive_Data_Pr[1]; //Temporary variable to save the data of the lower machine //临时变量，保存下位机数据
  ...
  if(count2 == Distance_DATA_size)
  {
    ...
    if(Distance_Data.rx[Distance_DATA_size-1]==Distance_TAIL)
    {
      ...
      if(error2==0)
      {
        ...
        // 解析红外距离数据 (13-14字节)
        float ir_distance_value = ((short)((Distance_Data.rx[13]<<8) | (Distance_Data.rx[14]))) / 1000.0f;
        
        
        
        // 解析按钮状态 (15-16字节) - 直接使用接收到的0和1
        bool button1_state = (Distance_Data.rx[15] == 0);  // 用户自定义按钮
        bool button2_state = (Distance_Data.rx[16] == 0);  // 防碰撞传感器

        //自检数据从
        temp_selfcheck = (uint32_t)(Distance_Data.rx[17]<<24|Distance_Data.rx[18]<<16|Distance_Data.rx[19]<<8|Distance_Data.rx[20]);
        ...
        std_msgs::Float32 ir_distance_msg;
        std_msgs::Bool button1_msg;
        std_msgs::Bool button2_msg;
              // 发布ROS话题
        ir_distance_msg.data = ir_distance_value;
        button1_msg.data = button1_state;
        button2_msg.data = button2_state;
        
        ir_distance_pub.publish(ir_distance_msg);
        button1_pub.publish(button1_msg);
        button2_pub.publish(button2_msg);
        distance_flag=true;
      }
    }
  }
...
turn_on_robot::turn_on_robot():Sampling_Time(0),Power_voltage(0)
{
  ...
  private_nh.param<std::string>("usart_port_name",  usart_port_name,  "/dev/ttyACM0"); //Fixed serial port number //固定串口号
  ...
  private_nh.param<std::string>("robot_type",       robot_type,    "Plus"); //机器人型号
  private_nh.param<std::string>("car_mode",         car_mode,    "S300_Mini"); //机器人型号
  ...
  distance_publisher= n.advertise<aihitplt_bringup::Supersonic>("Distance", 10); //创建超声波距离话题发布者
  Charging_publisher= n.advertise<std_msgs::Bool>("robot_charging_flag", 20); //Create robot charging flag position topic publisher //创建机器人充电标志位话题发布者
  Charging_current_publisher= n.advertise<std_msgs::Float32>("robot_charging_current", 20); //Create the charging current topic publisher //创建充电电流话题发布者
  RED_publisher     = n.advertise<std_msgs::Bool>("robot_red_flag", 20); //Create a topic publisher that finds the infrared signal flag `bit //创建寻找到红外信号标志位话题发布者
  SelfCheck_publisher = n.advertise<std_msgs::UInt32>("/self_check_data",20);
  SetRgb_Service = n.advertiseService("set_rgb_color", &turn_on_robot::Set_LightRgb_Callback,this);
  ir_distance_pub = n.advertise<std_msgs::Float32>("ir_distance", 10);
  button1_pub = n.advertise<std_msgs::Bool>("user_button", 10);
  button2_pub = n.advertise<std_msgs::Bool>("collision_sensor", 10);
  ...
  //红外对接速度话题订阅回调函数
  Red_Vel_Sub       = n.subscribe("red_vel",            100, &turn_on_robot::Red_Vel_Callback, this); 
  //Automatic recharge mode flag bit subscription callback function
  //自动回充模式标志位订阅回调函数
  Recharge_Flag_Sub = n.subscribe("robot_recharge_flag",100, &turn_on_robot::Recharge_Flag_Callback, this); 
  ...
  { 
    ...
    Stm32_Serial.flushInput(); //Clear the input buffer
  }
  catch (serial::IOException& e)
  {
    ROS_ERROR_STREAM("aihitplt_robot can not open serial port,Please check the serial port cable! "); //If opening the serial port fails, an error message is printed //如果开启串口失败，打印错误信息
  }
  if(Stm32_Serial.isOpen())
  {
    ROS_INFO_STREAM("aihitplt_robot serial port opened"); //Serial port opened successfully //串口开启成功提示
    ROS_INFO_STREAM(" - Port: " << Stm32_Serial.getPort());
  }
}
/**************************************
Date: January 28, 2021
