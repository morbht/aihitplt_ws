...
#include <aihitplt_bringup/Supersonic.h>
#include "aihitplt_bringup/SetRgb.h"
using namespace std;
...
class turn_on_robot
{
	...
	private:
		...
		void Security_Callback(const std_msgs::Int8 &Security_Flag);
		bool Set_LightRgb_Callback(aihitplt_bringup::SetRgb::Request &req,aihitplt_bringup::SetRgb::Response &res);
		
		ros::Publisher odom_publisher, imu_publisher, voltage_publisher,distance_publisher,Charging_publisher, Charging_current_publisher, RED_publisher,SelfCheck_publisher; //Initialize the topic publisher //初始化话题发布者
		ros::Publisher ir_distance_pub;
		ros::Publisher button1_pub;
		ros::Publisher button2_pub;
		ros::ServiceServer SetRgb_Service;
        ...
        string usart_port_name, robot_frame_id, gyro_frame_id, odom_frame_id,robot_type,car_mode; //Define the related variables //定义相关变量
