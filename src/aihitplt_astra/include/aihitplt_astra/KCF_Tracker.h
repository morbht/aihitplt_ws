...
#include <aihitplt_astra/KCFTrackerPIDConfig.h>
...
public:
    ...
    int center_x;
    KCFTracker tracker;
    dynamic_reconfigure::Server<aihitplt_astra::KCFTrackerPIDConfig> server;
    dynamic_reconfigure::Server<aihitplt_astra::KCFTrackerPIDConfig>::CallbackType f;

    ImageConverter(ros::NodeHandle &n);

    ~ImageConverter();

    void PIDcallback(aihitplt_astra::KCFTrackerPIDConfig &config, uint32_t level);
