...
base_path = os.path.expanduser("~/car_ws/src/car_astra/scripts/detection/")
######################### Detection ##########################
# load the COCO class names
with open(os.path.join(base_path,'object_detection_coco.txt'), 'r') as f: class_names = f.read().split('\n')
# get a different color array for each of the classes
COLORS = np.random.uniform(0, 255, size=(len(class_names), 3))
# load the DNN modelimage
model = cv.dnn.readNet(model=os.path.join(base_path,"frozen_inference_graph.pb"), config=os.path.join(base_path,"ssd_mobilenet_v2_coco.txt"), framework='TensorFlow')
...
net = cv.dnn.readNetFromTensorflow(os.path.join(base_path,"graph_opt.pb"))
