...
class color_Tracker:
    def __init__(self):
        ...
        self.dist = []
        self.encoding = ['32FC1']  # 适配仿真相机的编码格式
        ...
        self.angular_PID = (0.05, 0.0, 2.0)
        self.scale = 1.0  # 单位已经是米
        self.PID_init()
    ...
    def AstraFollowPID_callback(self, config, level):
        ...
        self.minDist = config['minDist']  # 单位为米
    ...
    def depth_img_Callback(self, msg):
        ...
        if self.Center_r != 0:
            ...
            # 修正边界检查语法
            if (0 <= int(self.Center_y) - 3 < 480 and 
                0 <= int(self.Center_y) + 3 < 480 and 
                0 <= int(self.Center_x) - 3 < 640 and 
                0 <= int(self.Center_x) + 3 < 640):
                
                distance = [
                    depthFrame[int(self.Center_y) - 3][int(self.Center_x) - 3],
                    depthFrame[int(self.Center_y) + 3][int(self.Center_x) - 3],
                    depthFrame[int(self.Center_y) - 3][int(self.Center_x) + 3],
                    depthFrame[int(self.Center_y) + 3][int(self.Center_x) + 3],
                    depthFrame[int(self.Center_y)][int(self.Center_x)]
                ]
                ...
                num_depth_points = 0
                for d in distance:
                    if 0.1 < d < 10.0:  # 对于32FC1，单位是米，合理范围是0.1到10米
                        distance_ += d
                        num_depth_points += 1
                ...
                print("Center_x: {}, Center_y: {}, distance: {}".format(
                    self.Center_x, self.Center_y, distance_))
    ...
    def execute(self, point_x, dist):
        if abs(self.prev_dist - dist) > 0.3:  # 单位为米
            self.prev_dist = dist
            return
        ...
        linear_x = self.linear_pid.compute(dist, self.minDist)  # 单位为米
        angular_z = self.angular_pid.compute(320, point_x)
        if abs(dist - self.minDist) < 0.03:  # 单位为米
            linear_x = 0
