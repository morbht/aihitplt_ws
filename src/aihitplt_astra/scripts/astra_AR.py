...
class simple_AR:
    def __init__(self):
        ...
        # 加载相机内参矩阵、畸变系数
        self.cameraMatrix =np.float32( [[1269.87401, 0., 403.42004],
        					[0. ,1291.12914, -344.09352],
							[0., 0., 1.]])

        self.distCoeffs =np.float32( [[-0.456484,0.193372, 0.035459, -0.001957, 0.]])
        self.objectPoints = np.zeros((9 * 6, 3), np.float32)
        self.objectPoints[:, :2] = np.mgrid[0.0:6.0, 0.0:9.0].T.reshape(-1, 2)
    ...
    def process(self, img, action):
        ...
        if retval:
            ...
            retval, rvec, tvec, inliers= cv.solvePnPRansac(self.objectPoints, corners, self.cameraMatrix, self.distCoeffs)
            # 输出图像点和雅可比矩阵
            # Output image points and Jacobian matrix
            image_Points, jacobian = cv.projectPoints(self.axis, rvec, tvec, self.cameraMatrix, self.distCoeffs, )
            img = self.draw(img, corners, image_Points)
        cv.putText(img, self.Graphics, (240, 30), cv.FONT_HERSHEY_SIMPLEX, 0.9, (0, 0, 255), 1)
        
        # Create a ROS Image message without cv_bridge
        img_msg = Image()
        img_msg.height = img.shape[0]
        img_msg.width = img.shape[1]
        img_msg.encoding = 'bgr8'
        img_msg.is_bigendian = 0
        img_msg.step = img.shape[1] * 3  # 3 bytes per pixel for BGR
