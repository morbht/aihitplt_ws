...
class simple_AR:
    def __init__(self):
        ...
        yaml_path = rospkg.RosPack().get_path("aihitplt_visual") + '/AR/astra.yaml'
        ...
        # 订阅图像话题
        self.sub_image = rospy.Subscriber(self.input_image_topic, Image, self.image_callback)
    ...
    def draw(self, img, corners, image_Points):
        ...
        if self.Graphics == "Triangle":
            ...
            cv.drawContours(img, [np.array([img_pts[58], img_pts[24], img_pts[35], img_pts[47]])], -1, (160, 252, 0), -1)
            cv.drawContours(img, [np.array([img_pts[40], img_pts[38], img_pts[21], img_pts[41]])], -1, (30, 144, 255), -1)
            ...
            cv.drawContours(img, [np.array([img_pts[14], img_pts[12], img_pts[13], img_pts[15]])], -1, (0, 199, 140), -1)
...
if __name__ == '__main__':
    ...
    rospy.loginfo("等待接收图像数据...")
    
    # 等待第一帧图像
    while not ar.frame_received and not rospy.is_shutdown():
        rate.sleep()
    ...
    while not rospy.is_shutdown():
        ...
        if ar.frame_received and ar.current_frame is not None:
            frame = ar.current_frame.copy()
